...
const Orders = () => {
  ...
  const getTableData = async () => {
    try {
      const orders = await axios.get(
        "https://bomadistapi.herokuapp.com/api/v1/order/"
      );
  ...
  const handleToggling = (event, index) => {
  ...
  const handleClick = async (e, itemId) => {
    ...
    try {
      await axios.delete(
        "https://bomadistapi.herokuapp.com/api/v1/order/" + itemId
      );
  ...
  return (
    <div class="sm:p-7 p-4">
      <div class="flex w-full items-center mb-7">
        ...
        <Stats sales={data?.map(item => item.sales)} />
